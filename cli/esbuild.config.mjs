 ...
 *
 * Bundles all workspace packages (@goitalia/*) into a single file.
...
// to npm and resolved at runtime (e.g. @goitalia/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@goitalia/")) {
      externals.add(name);
    }
  }
